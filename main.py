...
def test_all(testdata_loader, model):
    print(f"\n=== Test Data Loader Info ===")
    print(f"Number of batches: {len(testdata_loader)}")
    print(f"Dataset size: {len(testdata_loader.dataset)}")
    print(f"Batch size: {testdata_loader.batch_size}")
    print(f"Expected frames: {testdata_loader.dataset.n_frames}")
    print(f"FPS: {testdata_loader.dataset.fps}")
    ...
    with torch.no_grad():
        for i, (batch_xs, batch_ys, batch_toas) in enumerate(testdata_loader):
            ...
            if i == 0:
                print(f"\n=== First Batch Details ===")
                print(f"Features shape: {batch_xs.shape}")
                print(f"Labels shape: {batch_ys.shape}")
                print(f"TOA shape: {batch_toas.shape}")
                print(f"Labels (first 5): {batch_ys[:5]}")
                print(f"TOA (first 5): {batch_toas[:5]}")
            ...
            if i == 0:
                print(f"\n=== Model Output Check ===")
                print(f"Number of time steps: {len(all_outputs)}")
                first_output = all_outputs[0]
                print(f"First output shape: {first_output.shape}")
                print(f"First output (first sample):")
                sample = first_output[0].cpu().detach().numpy()
                print(f"  Raw logits: {sample}")
                print(f"  Softmax: {np.exp(sample) / np.sum(np.exp(sample))}")
                print(f"  Max logit: {np.max(sample)}, Min logit: {np.min(sample)}")
            ...
            for t in range(num_frames):
                pred = all_outputs[t]
                pred_np = pred.cpu().numpy() if pred.is_cuda else pred.detach().numpy()

                # 添加数值检查
                if np.any(np.abs(pred_np) > 100):
                    print(f"Warning: Large logits detected at time {t}: max={np.max(pred_np)}, min={np.min(pred_np)}")

                pred_frames[:, t] = np.exp(pred_np[:, 1]) / np.sum(np.exp(pred_np), axis=1)

            # 检查预测结果
            if i == 0:
                print(f"\n=== Prediction Stats (First Batch) ===")
                print(f"Shape: {pred_frames.shape}")
                print(f"Min: {pred_frames.min():.6f}, Max: {pred_frames.max():.6f}")
                print(f"Mean: {pred_frames.mean():.6f}, Std: {pred_frames.std():.6f}")
                print(f"First sample (all frames):\n{pred_frames[0]}")
    ...
    for k in range(p.epoch):
        ...
        for i, (batch_xs, batch_ys, batch_toas) in loop:
            ...
            if torch.isnan(losses['total_loss']) or torch.isinf(losses['total_loss']):
                print(f"\n!!! NaN or Inf detected in total_loss at iter {iter_cur}")
                print(f"  cross_entropy: {losses['cross_entropy']}")
                print(f"  auxloss: {losses['auxloss']}")
                print(f"  log: {losses['log']}")
                continue  # 跳过这个 batch

            losses['total_loss'].mean().backward()

            # 添加梯度裁剪并检查梯度
            total_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 10)

            # 定期输出梯度信息
            if iter_cur % 20 == 0:
                print(f"\nIter {iter_cur}: Gradient norm = {total_norm:.4f}")
                print(f"  Loss: {losses['total_loss'].item():.4f}")
                print(f"  CE: {losses['cross_entropy'].mean().item():.4f}")
                print(f"  Aux: {losses['auxloss'].mean().item():.4f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='./output',
                        help='The root directory for saving logs and models.')
